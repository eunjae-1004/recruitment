...
if history_file is not None and comps_df is not None:
    if st.button("📥 기존 결과 복구하기"):
        ...
        st.success("기존 작업 데이터를 성공적으로 불러왔습니다.")
...
if 'summary' in st.session_state:
    ...
    if menu == "1. 지원자 평가 점수표":
        ...
        st.download_button("📥 전체 점수표 다운로드", data=csv_full, file_name="total_applicant_scores.csv")
        ...
        for _, c_row in comps.iterrows():
            with st.expander(f"{c_row['company_name']} (정원: {c_row['capacity']})"):
                ...
                if not c_res.empty:
                    ...
                    st.download_button(f"📥 {c_row['company_name']} 결과 다운로드", c_res.to_csv(index=False).encode('utf-8-sig'), f"{c_row['company_name']}_results.csv")
        ...
        st.download_button("📥 2지망 구제자 명단 다운로드", guje_df.to_csv(index=False).encode('utf-8-sig'), "second_choice_matches.csv")
        ...
        for i, (_, row) in enumerate(comps.iterrows(), 1):
            ...
            html += f"<tr><td>{i}</td><td>{c}</td><td>{job_text}</td><td>{row['recruitment_headcount_total']}</td><td>{len(df[df['preferred_company_1'] == c])}</td><td>{row['capacity']}</td><td>{assigned_total}</td><td class='{'deficit-red' if deficit > 0 else ''}'>{deficit}</td>"
            ...
            for j in range(max_pf):
                if j < len(pf):
                    ...
                    if cand['assigned_company'] == "미배정": html += f"<td><div class='tooltip red-text'>{aid}<span class='tooltiptext'>{tip}<br>(미배정)</span></div></td>"
                    else: html += f"<td><div class='tooltip blue-text'>{aid}<span class='tooltiptext'>{tip}<br>(배정지: {cand['assigned_company']})</span></div></td>"
                else: html += "<td></td>"
            html += "</tr>"
        ...
        # 최종 저장용 버튼 (현황판 아래)
        st.download_button("💾 최종 결과 파일(작업 저장용) 다운로드", df.to_csv(index=False).encode('utf-8-sig'), "final_matching_results.csv")
else:
    st.info("CSV 파일을 업로드하거나 기존 결과를 불러와주세요.")

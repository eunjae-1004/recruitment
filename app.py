    ...
    if st.button("🚀 전체 배치 실행"):
        with st.spinner("점수 계산 및 자동 배치 중..."):
            ...
            for _, app in apps_df.iterrows():
                ...
                if t_cid:
                    target_comp_df = comps_df[comps_df['company_id'] == t_cid]
                    assigned_cname = target_comp_df['company_name'].values[0]
                    comp_row = target_comp_df.iloc[0]
                else:
                    assigned_cname = "미배정"
                    p1_name = app['preferred_company_1']
                    target_comp_df = comps_df[comps_df['company_name'] == p1_name]
                    # [IndexError 방지] 기업을 못 찾을 경우 첫 번째 기업 혹은 빈 객체로 대응

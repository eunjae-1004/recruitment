...
def clean_job_summary(text):
    ...
    return str(text)[:50] # 글자 제한을 조금 더 늘림
...
with st.expander("📂 데이터 파일 업로드 (이어서 작업하기 지원)", expanded=True):
    ...
    with col_u2:
        history_file = st.file_uploader("최종 결과 CSV 불러오기 (기존 작업 복구)", type="csv")
...
if 'summary' in st.session_state:
    ...
    if menu == "1. 지원자 평가 점수표":
        ...
        st.download_button("📥 전체 결과 CSV 다운로드", df.to_csv(index=False).encode('utf-8-sig'), "matching_summary.csv")
        ...
        .tooltip .tooltiptext {{ visibility: hidden; width: 180px; background-color: black; color: #fff; text-align: center; border-radius: 6px; padding: 6px; position: absolute; z-index: 100; bottom: 125%; left: 50%; margin-left: -90px; opacity: 0; transition: opacity 0.3s; font-size: 10px; }}
        .tooltip:hover .tooltiptext {{ visibility: visible; opacity: 1; }}
        
        /* 열폭 최적화 */
        .col-comp {{ min-width: 120px; }}
        .col-job {{ min-width: 360px; text-align: left; }} /* 사업장 대비 3배 폭 */
        .col-id-cell {{ min-width: 50px; }} /* 지원자 번호 칸이 줄어들지 않도록 설정 */
        </style>
        ...
        for i, (_, row) in enumerate(comps.iterrows(), 1):
            ...
            html += f"<td class='col-job'>{job_text}</td>"

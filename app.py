...
if 'assignments' in st.session_state:
    ...
    if menu == "1. 지원자 평가 점수표":
        ...
        sel_aid = st.selectbox("지원자 선택", df_results['applicant_id'].tolist(), 
                               format_func=lambda x: f"{x} ({df_results[df_results['applicant_id']==x]['applicant_name'].values[0]})")
        row = df_results[df_results['applicant_id'] == sel_aid].iloc[0]
        c_l, c_r = st.columns(2)
        with c_l:
            st.info(f"### {row['applicant_name']}\n**1지망**: {row['preferred_company_1']}\n**2지망**: {row['preferred_company_2_3']}\n**최종 배정**: {row['assigned_company']}")
            st.metric("종합 점수(1지망 기준)", f"{row['final_score']}점")
            st.success(f"**산출 근거**: {row['score_reason_summary']}")
        with c_r:
            st.write("항목별 상세 내역은 점수표(1번 페이지)를 참고해 주세요.")

    elif menu == "3. 기업별 매칭 결과":
        st.subheader("🏢 기업별 배정 현황")
        for _, c in comps_df.iterrows():
            with st.expander(f"{c['company_name']} (정원: {c['capacity']})"):
                c_res = df_results[df_results['assigned_company'] == c['company_name']]
        ...
        second_df = df_results[(df_results['assigned_company'] == df_results['preferred_company_2_3']) & (df_results['assigned_company'] != df_results['preferred_company_1']) & (df_results['assigned_company'] != "미배정")]
        st.dataframe(second_df, use_container_width=True)

    elif menu == "5. 종합 매칭 현황판 (상세 분석 및 수동배치)":
        st.subheader("📊 종합 매칭 현황판")
        
        # 가변 칸수 데이터 가공 (탈락자 점수순 정렬 포함)
        p1_data, p2_data, pf_data = {}, {}, {}
        for c in comps_df['company_name']:
            c = c.strip()
            p1_data[c] = df_results[(df_results['assigned_company'] == c) & (df_results['preferred_company_1'] == c)]['applicant_id'].tolist()
            p2_data[c] = df_results[(df_results['assigned_company'] == c) & (df_results['preferred_company_1'] != c) & (df_results['assigned_company'] != "미배정")]['applicant_id'].tolist()
            
            # [수정] 1차 지원 탈락자를 점수 내림차순으로 정렬
            pf_df = df_results[(df_results['preferred_company_1'] == c) & (df_results['assigned_company'] != c)]
        ...
        html = f"<table class='m-table'><tr class='bg-gray'><th rowspan='2'>연번</th><th rowspan='2'>지원 사업장</th><th rowspan='2'>지원 분야</th><th rowspan='2'>채용</th><th rowspan='2'>지원</th><th rowspan='2'>배수</th><th rowspan='2'>배정</th><th rowspan='2'>부족</th><th colspan='{max_p1}'>1차 배정</th><th colspan='{max_p2}'>2차 배정</th><th colspan='{max_pf}'>1차 탈락 (점수순)</th></tr><tr class='bg-gray'>"
        ...
        for i, (_, row) in enumerate(comps_df.iterrows(), 1):
            ...
            html += f"<tr><td>{i}</td><td>{c}</td><td>{row.get('recruitment_job_groups', '전체')}</td><td>{row['recruitment_headcount_total']}</td><td>{len(df_results[df_results['preferred_company_1'] == c])}</td><td>{row['capacity']}</td><td>{assigned_cnt}</td><td class='{'deficit-red' if deficit > 0 else ''}'>{deficit}</td>"
            ...
            for j in range(max_pf):
                if j < len(pf):
                    aid = pf[j]
                    a_info = df_results[df_results['applicant_id'] == aid].iloc[0]
        ...
        unassigned_list = df_results[df_results['assigned_company'] == "미배정"]['applicant_id'].tolist()
        if unassigned_list:
            c_m1, c_m2, c_m3 = st.columns([2, 2, 1])
            with c_m1: s_aid = st.selectbox("수동 배치 지원자", unassigned_list)
            with c_m2: s_comp = st.selectbox("배치할 기업", comps_df['company_name'].tolist())
            with c_m3:
                if st.button("배정 확정"):
                    new_cid = comps_df[comps_df['company_name'] == s_comp]['company_id'].values[0]
                    st.session_state['assignments'][s_aid] = new_cid
                    st.rerun()
        else: st.write("모든 지원자 배정 완료")
